...
internal class FixKotlinJsonAdapter<T>(
    ...
    for (i in 0 until constructorSize) {
      if (values[i] === ABSENT_VALUE) {
        when {
          ...
          //Determine whether the json object is an empty object
          (i == nullParameters) -> {
  ...
  override fun toString() = "FixKotlinJsonAdapter(${constructor.returnType})"
...
public class FixKotlinJsonAdapterFactory : JsonAdapter.Factory {
  override fun create(type: Type, annotations: MutableSet<out Annotation>, moshi: Moshi):
    ...
    val bindingsByName = LinkedHashMap<String, FixKotlinJsonAdapter.Binding<Any, Any?>>()
    ...
    for (property in rawTypeKotlin.memberProperties) {
      ...
      bindingsByName[property.name] = FixKotlinJsonAdapter.Binding(
        jsonName,
        adapter,
    ...
    val bindings = ArrayList<FixKotlinJsonAdapter.Binding<Any, Any?>?>()
    ...
    return FixKotlinJsonAdapter(constructor, bindings, nonIgnoredBindings, options).nullSafe()
  }
}
